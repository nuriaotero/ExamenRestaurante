...
<body>
    <div class="container">
        ...
        <form method="POST" action="<?= BASE_URL ?>?url=reserva/crear">
            <div class="form-group">
                <label for="mesa_id">Mesa:</label>
                <select name="mesa_id" id="mesa_id" required>
                    <option value="">Selecciona una mesa</option>
                    <?php foreach ($mesas as $mesa): ?>
                        <option value="<?= $mesa->id ?>">#<?= htmlspecialchars($mesa->numero) ?> (<?= htmlspecialchars($mesa->plazas) ?> plazas)</option>
                    <?php endforeach; ?>
                </select>
            </div>
            
            <div class="form-group">
                <label for="tramo_horario_id">Horario:</label>
                <select name="tramo_horario_id" id="tramo_horario_id" required>
                    <option value="">Selecciona un horario</option>
                    <?php foreach ($tramos as $tramo): ?>
                        <option value="<?= $tramo->id ?>"><?= htmlspecialchars($tramo->nombre) ?> (<?= htmlspecialchars($tramo->hora_inicio) ?> - <?= htmlspecialchars($tramo->hora_fin) ?>)</option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="date" name="fecha_reserva" id="fecha_reserva" required min="<?= date('Y-m-d') ?>">
            </div>
            
            <div class="form-group">
                <label for="numero_personas">Número de Personas:</label>
                <input type="number" name="numero_personas" id="numero_personas" min="1" max="20" required>
            </div>
